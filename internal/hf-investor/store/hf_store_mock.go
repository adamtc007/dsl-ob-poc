...
type MockHedgeFundInvestorStore struct {
	...
	documentRequirements map[uuid.UUID]*HedgeFundDocumentRequirement
...
func (m *MockHedgeFundInvestorStore) GetPositionsAsOf(ctx context.Context, asOfDate time.Time, filters *PositionFilters) ([]PositionAsOf, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()

	// Simulate the position as-of query by aggregating register events
	positionMap := make(map[string]*PositionAsOf)

	for _, event := range m.registerEvents {
		// Check if event is on or before the as-of date
		if event.ValueDate.After(asOfDate) {
			continue
		}

		// Find the lot to get position identifiers
		lot, exists := m.registerLots[event.LotID]
		if !exists {
			continue
		}
		...
		if filters != nil {
			if filters.InvestorID != nil && lot.InvestorID != *filters.InvestorID {
				continue
			}
			if filters.FundID != nil && lot.FundID != *filters.FundID {
				continue
			}
			if filters.ClassID != nil && lot.ClassID != *filters.ClassID {
				continue
			}
			if filters.SeriesID != nil {
				if lot.SeriesID == nil || *lot.SeriesID != *filters.SeriesID {
					continue
				}
			}
		}
		...
		if lot.SeriesID != nil {
			seriesKey = lot.SeriesID.String()
		}
		key := lot.InvestorID.String() + "|" + lot.FundID.String() + "|" + lot.ClassID.String() + "|" + seriesKey

		// Update position
		if pos, posExists := positionMap[key]; posExists {
			pos.Units += event.DeltaUnits
		} else {
			positionMap[key] = &PositionAsOf{
				InvestorID: lot.InvestorID,
				FundID:     lot.FundID,
				ClassID:    lot.ClassID,
				SeriesID:   lot.SeriesID,
				Units:      event.DeltaUnits,
			}
		}
	}

	// Convert to slice and filter by minimum units
	result := make([]PositionAsOf, 0, len(positionMap))
	for _, pos := range positionMap {
		if filters != nil && filters.MinUnits != nil && pos.Units < *filters.MinUnits {
			continue
		}
		if pos.Units != 0 { // Only include non-zero positions
...
func (m *MockHedgeFundInvestorStore) GetPipelineFunnel(ctx context.Context, filters *PipelineFilters) ([]PipelineStatusCount, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	for _, investor := range m.investors {
		// Apply filters if provided
		if filters != nil {
			if filters.InvestorType != nil && investor.Type != *filters.InvestorType {
				continue
			}
			if filters.Domicile != nil && investor.Domicile != *filters.Domicile {
				continue
			...
			if filters.Source != nil {
				if investor.Source == nil || *investor.Source != *filters.Source {
					continue
				}
			}
			if filters.DateFrom != nil && investor.CreatedAt.Before(*filters.DateFrom) {
				continue
			}
			if filters.DateTo != nil && investor.CreatedAt.After(*filters.DateTo) {
				continue
			}
		}

		statusCounts[investor.Status]++
	}

	// Convert to slice and sort by status
	result := make([]PipelineStatusCount, 0, len(statusCounts))
	for status, count := range statusCounts {
		result = append(result, PipelineStatusCount{
			Status:    status,
			Investors: count,
		})
	}

	// Sort by status to match SQL ORDER BY
...
func (m *MockHedgeFundInvestorStore) GetOutstandingKYC(ctx context.Context, filters *KYCRequirementFilters) ([]KYCRequirement, error) {
	// Mock implementation - would normally join document_requirements with investors table
	result := []KYCRequirement{
		{
			InvestorID:   uuid.New(),
			InvestorCode: "INV001",
			InvestorName: "Sample Corporation",
			DocType:      "PASSPORT",
			Status:       "REQUESTED",
			Priority:     "HIGH",
			RequestedAt:  time.Now().AddDate(0, 0, -10),
			DueAt:        func() *time.Time { t := time.Now().AddDate(0, 0, -2); return &t }(),
			DaysOverdue:  func() *int { days := 2; return &days }(),
			Source:       func() *string { s := "KYC_ONBOARD"; return &s }(),
		},
		{
			InvestorID:   uuid.New(),
			InvestorCode: "INV002",
			InvestorName: "Another Fund Ltd",
			DocType:      "BANK_STATEMENT",
			Status:       "OVERDUE",
			Priority:     "CRITICAL",
			RequestedAt:  time.Now().AddDate(0, 0, -30),
			DueAt:        func() *time.Time { t := time.Now().AddDate(0, 0, -15); return &t }(),
			DaysOverdue:  func() *int { days := 15; return &days }(),
			Source:       func() *string { s := "PERIODIC_REFRESH"; return &s }(),
		},
	}

	// Apply filters
	if filters != nil {
		filtered := make([]KYCRequirement, 0, len(result))
		for i := range result {
			req := result[i]
			if filters.InvestorID != nil && req.InvestorID != *filters.InvestorID {
				continue
			}
			if filters.DocType != nil && req.DocType != *filters.DocType {
				continue
			}
			if filters.Status != nil && req.Status != *filters.Status {
				continue
			}
			if filters.Priority != nil && req.Priority != *filters.Priority {
				continue
			}
			if filters.OverdueOnly != nil && *filters.OverdueOnly && req.DaysOverdue == nil {
				continue
			}
...
func (m *MockHedgeFundInvestorStore) CreateDocumentRequirement(ctx context.Context, req *HedgeFundDocumentRequirement) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	if m.documentRequirements == nil {
		m.documentRequirements = make(map[uuid.UUID]*HedgeFundDocumentRequirement)
	}

	req.RequirementID = uuid.New()
	req.CreatedAt = time.Now().UTC()
	req.UpdatedAt = time.Now().UTC()

	m.documentRequirements[req.RequirementID] = req
	return nil
}
func (m *MockHedgeFundInvestorStore) GetDocumentRequirements(ctx context.Context, investorID uuid.UUID) ([]HedgeFundDocumentRequirement, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()

	result := make([]HedgeFundDocumentRequirement, 0)
	for _, req := range m.documentRequirements {
...
func (m *MockHedgeFundInvestorStore) UpdateDocumentRequirement(ctx context.Context, req *HedgeFundDocumentRequirement) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	if _, exists := m.documentRequirements[req.RequirementID]; !exists {
		return fmt.Errorf("document requirement %s not found", req.RequirementID)
	}
...
func (m *MockHedgeFundInvestorStore) FulfillDocumentRequirement(ctx context.Context, requirementID, documentID uuid.UUID) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	req, exists := m.documentRequirements[requirementID]
	if !exists {
		return fmt.Errorf("document requirement %s not found", requirementID)
	}

	now := time.Now().UTC()
	req.FulfillingDocumentID = &documentID
	req.FulfilledAt = &now
	req.Status = DocRequirementStatusSubmitted
	req.UpdatedAt = now
