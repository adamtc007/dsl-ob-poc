...
type HedgeFundInvestorStore interface {
	...
	GetPositionsAsOf(ctx context.Context, asOfDate time.Time, filters *PositionFilters) ([]PositionAsOf, error)

	// Operations Dashboard
	GetPipelineFunnel(ctx context.Context, filters *PipelineFilters) ([]PipelineStatusCount, error)
	GetOutstandingKYC(ctx context.Context, filters *KYCRequirementFilters) ([]KYCRequirement, error)

	// Document Requirements
	CreateDocumentRequirement(ctx context.Context, req *HedgeFundDocumentRequirement) error
	GetDocumentRequirements(ctx context.Context, investorID uuid.UUID) ([]HedgeFundDocumentRequirement, error)
	UpdateDocumentRequirement(ctx context.Context, req *HedgeFundDocumentRequirement) error
	FulfillDocumentRequirement(ctx context.Context, requirementID, documentID uuid.UUID) error
...
// PositionAsOf represents a position projection as of a specific date
...
// PositionFilters for filtering position as-of queries
...
// PipelineStatusCount represents investor count by status for pipeline funnel
type PipelineStatusCount struct {
	Status    string `json:"status" db:"status"`
	Investors int    `json:"investors" db:"investors"`
}

// PipelineFilters for filtering pipeline funnel queries
...
// HedgeFundDocumentRequirement represents a document requirement
type HedgeFundDocumentRequirement struct {
	...
	FulfillingDocumentID *uuid.UUID `json:"fulfilling_document_id,omitempty" db:"fulfilling_document_id"`
	Source               *string    `json:"source,omitempty" db:"source"`
	RequirementsNotes    *string    `json:"requirements_notes,omitempty" db:"requirements_notes"`
...
// KYCRequirement represents outstanding KYC requirements for dashboard
type KYCRequirement struct {
	...
	RequestedAt  time.Time  `json:"requested_at" db:"requested_at"`
	DueAt        *time.Time `json:"due_at,omitempty" db:"due_at"`
	DaysOverdue  *int       `json:"days_overdue,omitempty" db:"days_overdue"`
	Source       *string    `json:"source,omitempty" db:"source"`
}

// KYCRequirementFilters for filtering outstanding KYC queries
type KYCRequirementFilters struct {
	InvestorID  *uuid.UUID
	DocType     *string
	Status      *string
	Priority    *string
...
// Document Requirement Status constants
const (
	DocRequirementStatusRequested = "REQUESTED"
	DocRequirementStatusSubmitted = "SUBMITTED"
	DocRequirementStatusApproved  = "APPROVED"
	DocRequirementStatusRejected  = "REJECTED"
	DocRequirementStatusOverdue   = "OVERDUE"
	DocRequirementStatusWaived    = "WAIVED"
)

// Document Requirement Priority constants
const (
	DocRequirementPriorityLow      = "LOW"
	DocRequirementPriorityMedium   = "MEDIUM"
	DocRequirementPriorityHigh     = "HIGH"
	DocRequirementPriorityCritical = "CRITICAL"
)

// Document Requirement Source constants
const (
	DocRequirementSourceKYCOnboard       = "KYC_ONBOARD"
	DocRequirementSourcePeriodicRefresh  = "PERIODIC_REFRESH"
	DocRequirementSourceRegulatoryChange = "REGULATORY_CHANGE"
	DocRequirementSourceManualRequest    = "MANUAL_REQUEST"
	DocRequirementSourceRiskReview       = "RISK_REVIEW"
)
